...
void caesar_cipher(unsigned char plaintext[MAX_STR_LEN], unsigned char passPic[50][50])
{
 ...
 while (plaintext[i] != '\0') {
     // Apply the displacement from the image
     plaintext[i] = plaintext[i] + passPic[j][k];

     // Ensure the character does not wrap around to become '\0'
     if (plaintext[i] == '\0') {
         plaintext[i] += 1;  // Adjust to avoid '\0'
     }
     ...
     if (k == 50) {
         ...
         j++;
         if (j == 50) {
             j = 0;  // Wrap around to the beginning of the image
         }
     }
...
void caesar_decipher(unsigned char cryptotext[MAX_STR_LEN], unsigned char passPic[50][50])
{
 ...
 while (cryptotext[i] != '\0') {
     // Apply the reverse displacement from the image
     cryptotext[i] = cryptotext[i] - passPic[j][k];

     // Ensure the character does not wrap around to become '\0'
     if (cryptotext[i] == '\0') {
         cryptotext[i] -= 1;  // Adjust to avoid '\0'
     }
